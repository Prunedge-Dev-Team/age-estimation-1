...
class Tree(nn.Module):
    def __init__(self, depth, feature_length, vector_length, use_cuda = True):
        ...
        if not use_cuda:
            ...
            self.mean = Parameter(torch.from_numpy(self.mean).type(torch.FloatTensor).cpu(), requires_grad=False)
            self.covmat = Parameter(torch.from_numpy(self.covmat).type(torch.FloatTensor).cpu(), requires_grad=False)
            self.covmat_inv = Parameter(torch.from_numpy(self.covmat_inv).type(torch.FloatTensor).cpu(), requires_grad=False)
            self.factor = Parameter(torch.from_numpy(self.factor).type(torch.FloatTensor).cpu(), requires_grad=False)            
...
class Forest(nn.Module):
    ...
    def __init__(self, n_tree, tree_depth, feature_length, vector_length, use_cuda = False):
        ...
        for _ in range(n_tree):
            tree = Tree(tree_depth, feature_length, vector_length)
